...
IMPROVED: Handles enums properly by converting to strings for serialization.
"""
...
def _interface_to_dict(interface: InterfaceInfo) -> Dict[str, Any]:
    ...
    IMPROVED: Converts enums to strings explicitly for JSON/CSV export.
    """
    return {
        "name": interface.name,
        "interface_type": str(interface.interface_type),
        ...
        "dns_leak_status": str(interface.dns_leak_status),
...
def export_to_json(
    ...
    if include_metadata:
        ...
        vpn_interfaces = [i for i in interfaces if str(i.interface_type) == "vpn"]
        leak_detected = any(str(i.dns_leak_status) == "LEAK" for i in interfaces)
