...
def get_internal_ipv4(iface_name: str) -> str:
    ...
    """
    logger.debug(f"[{iface_name}] Querying IPv4 address")
    
    if not (output := run_command(["ip", "-4", "addr", "show", iface_name])):
        logger.debug(f"[{iface_name}] No IPv4 configuration found")
        return "N/A"
    
    for line in output.split("\n"):
        if (line := line.strip()).startswith("inet "):
            parts = line.split()
            if len(parts) >= 2:
                # Format is "inet 192.168.1.100/24"
                ip_with_mask = parts[1]
                ipv4 = ip_with_mask.split("/")[0]
                logger.debug(f"[{iface_name}] IPv4: {ipv4}")
                return ipv4
    
    logger.debug(f"[{iface_name}] No IPv4 address assigned")
    return "N/A"
...
def get_internal_ipv6(iface_name: str) -> str:
    ...
    """
    logger.debug(f"[{iface_name}] Querying IPv6 address")
    
    if not (output := run_command(["ip", "-6", "addr", "show", iface_name])):
        logger.debug(f"[{iface_name}] No IPv6 configuration found")
        return "N/A"
    
    for line in output.split("\n"):
        if (line := line.strip()).startswith("inet6 "):
            parts = line.split()
            if len(parts) >= 2:
                ...
                if "deprecated" in line or "temporary" in line:
                    continue
                
                # Return first global unicast address
                if "scope global" in line:
                    logger.debug(f"[{iface_name}] IPv6: {ip_addr}")
                    return ip_addr
    
    logger.debug(f"[{iface_name}] No global IPv6 address assigned")
    return "N/A"
...
def get_default_gateway(iface_name: str) -> str:
    ...
    """
    logger.debug(f"[{iface_name}] Querying default gateway")
    
    if not (output := run_command(["ip", "route", "show", "dev", iface_name])):
        logger.debug(f"[{iface_name}] No routes found")
        return "NONE"
    ...
    for line in output.split("\n"):
        if line.startswith("default via "):
            ...
            if len(parts) >= 3:
                ...
                gateway = parts[2]
                logger.debug(f"[{iface_name}] Gateway: {gateway}")
                return gateway
    
    logger.debug(f"[{iface_name}] No default gateway configured")
    return "NONE"
...
def get_route_metric(iface_name: str) -> str:
    ...
    """
    logger.debug(f"[{iface_name}] Querying route metric")
        ...
        if line.startswith("default via "):
            ...
            if "metric" in parts:
                ...
                if metric_index + 1 < len(parts):
                    ...
                    logger.debug(f"[{iface_name}] Metric: {metric}")
                    return metric
            
            # No explicit metric shown - kernel is using its default
            logger.debug(f"[{iface_name}] Metric: DEFAULT (kernel-assigned)")
            return "DEFAULT"
    
    logger.debug(f"[{iface_name}] No default route")
    return "NONE"
...
def get_active_interface() -> Optional[str]:
    ...
    if not (output := run_command(["ip", "route", "show", "default"])):
    ...
    for line in output.split("\n"):
        ...
        if len(parts) >= 4 and parts[0] == "default" and parts[1] == "via" and "dev" in parts:
            dev_index = parts.index("dev")
            if dev_index + 1 < len(parts):
                iface = parts[dev_index + 1]
                logger.debug(f"Active interface: {iface}")
                return iface
