...
| none    | `isolated`         | VPN active; DNS stripped by VPN provider (no servers configured on this interface) |
| none    | `not_applicable`   | Interface excluded from DNS leak detection (structural or operational); includes VPN kill-switch interfaces -- GREEN requires active VPN DNS |
