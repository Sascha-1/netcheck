...
def format_output(interfaces: List[InterfaceInfo]) -> None:
    ...
    IMPROVED: Handles enums directly without str() conversions.
    """
    ...
    for interface in interfaces:
        # Convert interface_type to string for display (enums are already strings)
        iface_type_str = str(interface.interface_type)
        ...
        # Color coding based on status (compare with enum values)
        dns_leak_str = str(interface.dns_leak_status)
        iface_type_str_cmp = str(interface.interface_type)
        external_ipv4_str = str(interface.external_ipv4)

        if dns_leak_str == str(DnsLeakStatus.LEAK):
            row_text = f"{Colors.YELLOW}{row_text}{Colors.RESET}"
        elif dns_leak_str == str(DnsLeakStatus.WARN):
            row_text = f"{Colors.YELLOW}{row_text}{Colors.RESET}"
        elif iface_type_str_cmp == str(InterfaceType.VPN) and dns_leak_str == str(DnsLeakStatus.OK):
            row_text = f"{Colors.GREEN}{row_text}{Colors.RESET}"
        elif iface_type_str_cmp == str(InterfaceType.VPN) and external_ipv4_str != str(DataMarker.NOT_APPLICABLE):
            row_text = f"{Colors.GREEN}{row_text}{Colors.RESET}"
        elif interface.carries_vpn:
            row_text = f"{Colors.CYAN}{row_text}{Colors.RESET}"
        elif external_ipv4_str != str(DataMarker.NOT_APPLICABLE):
